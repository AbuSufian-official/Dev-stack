...
const MainCard = ({ obj, stackData, setStackData}) => {
  ...
  function handelSelectStackBtn(value) {
   ...
   function isStackhasSelectedDataFunc(e){
      let aa=Boolean(stackData.find((n)=>n==obj)) 
      console.log(aa)
      return aa
    } 
      ...
      <div className="card-body">
        ...
        <div className="mt-2">
          <button onClick={() => handelSelectStackBtn(obj)}
            ...
            className={`
          ${isadd ? `bg-[#d91b7d34]` : `bg-[#080d1b]`} 
          ${isadd ? `text-[#d91b7d]` : `  text-white`}    mt-7  w-full   rounded-[15px]  justify-center  px-5  py-3  items-center  font-bold   text-center   flex   flex-row  transition-all  duration-300  hover:-translate-y-1  hover:shadow-[0_10px_25px_rgba(8,13,27,0.18)] active:translate-y-0
        `}>{ isadd? (<span className="flex items-center gap-3"><FaCheck />Add to Stack </span>) : 'Add to Stack'}</button>
        </div>
      </div>
    </div>
